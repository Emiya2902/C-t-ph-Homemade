...
window.GameCore = {
  ...
  chanceCards: [
    ...
    { title: "💸 Phạt Giao Thông", text: "Đi sai làn đường tại Cầu Sài Gòn, nộp phạt $50.", action: "MONEY", amount: -50 },
    ...
    { title: "✈️ Du Lịch Đại học Kinh Tế", text: "Đi thẳng đến Đại học Kinh Tế (#15).", action: "MOVE_TO", target: 15, getGoBonus: false },
    { title: "⚡ Trúng Thầu Dự Án", text: "Trúng gói thầu công trình xanh, nhận ngay $100 tiền thưởng.", action: "MONEY", amount: 100 },
    { title: "❄️ Đóng Băng Đất Địch", text: "Đóng băng ô đất đắt nhất của đối thủ cho đến khi có người đáp vào.", action: "FREEZE_ENEMY_TILE" },
    { title: "🛡️ Bảo Vệ Lãnh Địa", text: "Bảo vệ ô đất của bạn khỏi phá nhà và tráo đất cho đến khi có người đáp vào.", action: "PROTECT_MY_LAND" },
    { title: "🔥 Tăng Giá Thuê", text: "Tăng 1.5x tiền thuê ô đất đắt nhất của bạn cho đến khi có người đáp vào.", action: "BOOST_RENT_TEMP" },
  ...
  fortuneCards: [
    ...
    { title: "🛍️ Mua Sắm Chợ Kim Biên", text: "Mua quà lưu niệm tại Chợ Kim Biên, thanh toán $45.", action: "MONEY", amount: -45 },
  ...
  init() {
    ...
    this.state.lastStationRoutePath = [];
    this.state.stationEvents = [];
  ...
  activateCenterBuff(player) {
    ...
    this.state.pendingCenterBuffNotification = message;
    return buff;
  },
  ...
  getMovementPath(player, steps) {
    ...
    if (this.state.crossRoute && this.state.crossRoute.playerId === player.id) {
      ...
      if (cursor >= route.length - 1) {
        handledStations.add(route[route.length - 1]);
        this.state.crossRoute = null;
    ...
    return { path, position, crossedToStation, stationEvents };
  },
  ...
  collectRent(payer, tile) {
    ...
    if (payer.money < 0) {
      ...
      this.addLog(`💸 ${payer.name} trả $${payableRent} tiền thuê cho ${recipient.name} và đang nợ $${Math.abs(payer.money)}! Hãy bán/cầm cố tài sản trước khi kết thúc lượt.`);
    } else {
  ...
  applyCardEffect() {
    ...
    switch (card.action) {
      ...
      case "MOVE_STEPS": {
        ...
        const movement = steps > 0 ? this.movePlayerOuter(p, p.position + steps) : { startPos: p.position, movementPath: [] };
        cardMovementStart = movement.startPos;
        cardMovementPath = movement.movementPath;
        if (steps <= 0) p.position = (p.position + steps + 40) % 40;
