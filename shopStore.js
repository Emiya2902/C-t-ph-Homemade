...
(function () {
  ...
  function getTileTargets(card, player) {
    return card.id === 'TELEPORT_FORWARD'
      ...
      : window.GameCore.state.board.filter(tile => tile.type === 'PROPERTY' && tile.price && (
        card.id === 'SLIDE_OIL' ||
  ...
  function buyCard(cardId) {
    ...
    if (special) {
      ...
      // Thẻ có tác động ngay hoặc cần chọn mục tiêu không được cất vào kho.
      if (card.category === 'ATTACK' || card.requireTarget) {
        window.GameCore.addLog(`⚔️ ${player.name} mua và kích hoạt ngay thẻ [${card.title}].`);
        if (card.requireTarget) {
          return renderTargetSelector(card, player);
        }
  ...
  function executeCard(card, player, target) {
    ...
    switch (card.id) {
      ...
      case 'BUS_TICKET': {
        const stations = core.state.board.filter(tile => tile.type === 'RAILROAD');
        const next = stations.sort((a, b) =>
          ((a.id - player.position + 40) % 40) - ((b.id - player.position + 40) % 40)
        )[0];
        if (next) {
          const movement = core.movePlayerOuter(player, next.id);
          landing = { startPos: movement.startPos, movementPath: movement.movementPath };
        }
        break;
      }
      ...
      case 'PROPERTY_SABOTAGE': if (target && target.owner !== player.id && target.type === 'PROPERTY' && target.houses > 0 && !target.permanentProtection && !blocked({ type: 'SABOTAGE', tile: target })) target.houses -= 1; break;
      case 'TELEPORT_FORWARD': if (target) {
        const movement = core.movePlayerOuter(player, target.id);
        landing = { startPos: movement.startPos, movementPath: movement.movementPath };
      } break;
      case 'BOOST_RENT_1_5': { const tile = core.state.board.find(item => item.owner === target.id && item.price); if (tile) tile.boostTurns = true; break; }
      case 'FORCE_MORTGAGE': { const tile = core.state.board.find(item => item.owner === richest?.id && item.houses === 0 && item.mortgaged); if (tile && richest && !core.interceptAttack(player, richest, { type: 'FORCE_MORTGAGE', tile })) tile.mortgaged = false; break; }
      ...
      case 'PULL_RICHEST': {
        const tile = tiles.slice().sort((a, b) => b.price - a.price)[0];
        if (tile && richest && !core.interceptAttack(player, richest, { type: 'FORCED_MOVE', tile })) {
          const startPos = richest.position;
          richest.position = tile.id;
          core.processTileLanding(richest, { startPos, movementPath: [] });
        }
        break;
      }
      case 'PROTECT_LAND_PERMANENT': if (tiles.length) tiles[0].permanentProtection = true; break;
      case 'SWAP_UNBUILT_TILE': { const mine = tiles.find(item => !item.houses && !item.permanentProtection); const theirs = target && core.state.board.find(item => item.owner === target.id && item.price && !item.houses && !item.permanentProtection); if (mine && theirs && !blocked({ type: 'LAND_SWAP', tile: theirs })) { mine.owner = target.id; theirs.owner = player.id; } break; }
      case 'EARTHQUAKE_STRIKE': if (target && !blocked({ type: 'EARTHQUAKE' })) core.state.board.filter(item => item.owner === target.id && item.type === 'PROPERTY' && item.houses > 0 && !item.permanentProtection).forEach(item => { item.houses -= 1; }); break;
      case 'MIND_CONTROL': {
        const tile = tiles.slice().sort((a, b) => b.price - a.price)[0];
        if (tile && target && !blocked({ type: 'MIND_CONTROL', tile })) {
          const startPos = target.position;
          target.position = tile.id;
          target.shopRentReduction = 0.3;
          core.processTileLanding(target, { startPos, movementPath: [] });
        }
        break;
      }
      ...
      case 'POSITION_SWAP': {
        const length = core.state.board.length;
        const distance = Math.min(
          Math.abs(target.position - player.position),
          length - Math.abs(target.position - player.position)
        );
